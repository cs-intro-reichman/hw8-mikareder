...
public class Network {
    ...
    *  Otherwise, creates a new user with the given name, adds the user to this network, and returns true. */
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        if (user.follows(users[i].getName())) continue;

        int mutual = users[i].countMutual(user);
        if (mutual > max) {
            max = mutual;
            best = users[i];
        }
    ...
    for (int i = 0; i < userCount; i++) {
        ...
        for (int j = 0; j < userCount; j++) {
            String candidate = users[j].getName();
            if (users[i].follows(candidate)) {
                ans += candidate + " ";
            }
        }
